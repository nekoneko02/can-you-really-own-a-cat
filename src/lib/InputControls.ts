...
export class InputControls {
  // private sessionManager: UserSessionManager; // 新プロジェクトでは使用しない
  private config: InputConfig;

  constructor() {
    // this.sessionManager = UserSessionManager.getInstance(); // 新プロジェクトでは使用しない
    this.config = {
      enablePetting: true,
  ...
  setupMouseControls(
    ...
    cat: any, // Cat 型は新プロジェクトで異なる
    toy: any, // Toy 型は新プロジェクトでは使用しない
  ...
  setupToyMovement(
    ...
    scene.input.on('pointermove', (pointer: Phaser.Input.Pointer) => {
      if (toySprite && toySprite.active && toy) {
        ...
        // this.sessionManager.recordAction(SessionAction.mouseMove(pointer.x, pointer.y)); // 新プロジェクトでは使用しない
  ...
  setupPettingInteraction(
    ...
    scene.input.on('pointerdown', (pointer: Phaser.Input.Pointer) => {
      ...
      if (distance < this.config.pettingDistance) {
        // this.sessionManager.recordAction(SessionAction.petCat(pointer.x, pointer.y)); // 新プロジェクトでは使用しない
