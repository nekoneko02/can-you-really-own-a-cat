/**
 * シナリオ完了API
 * POST /api/scenarios/{scenarioSlug}/complete
 */

import { NextRequest, NextResponse } from 'next/server';
import { isValidScenarioSlug, validateCompleteRequest } from '@/lib/api/validation';
import { getSurveyStorage } from '@/lib/api/surveyStorage';
import { ERROR_CODES, type CompleteRequest, type CompleteResponse, type ErrorResponse } from '@/lib/api/types';
...
export async function POST(
  ...
  try {
    ...
    // リクエストのバリデーション
    const validationError = validateCompleteRequest(body);
    if (validationError) {
